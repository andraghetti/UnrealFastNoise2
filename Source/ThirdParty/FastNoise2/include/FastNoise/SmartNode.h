...
public:
    ...
    constexpr SmartNode(std::nullptr_t = nullptr) noexcept : mReferenceId(SmartNodeManager::kInvalidReferenceId), mPtr(nullptr)
    {
    }
...
private:
    ...
    explicit SmartNode(T* ptr)
        : mReferenceId(ptr ? SmartNodeManager::GetReference(ptr) : SmartNodeManager::kInvalidReferenceId), mPtr(ptr)
    {
        if (mReferenceId != SmartNodeManager::kInvalidReferenceId)
        {

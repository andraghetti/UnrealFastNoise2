...
{
    static_assert(std::is_base_of<Generator, T>::value,
    ...
    static_assert(std::is_member_function_pointer<decltype(&T::GetMetadata)>::value,
        "Cannot create abstract node class, use a derived class, for example: Fractal -> FractalFBm");
...
FASTNOISE_API SmartNode<> NewFromEncodedNodeTree(
    const char* encodedNodeTreeString, FastSIMD::eLevel maxSimdLevel = FastSIMD::Level_Null);
}    // namespace FastNoise

...
public:
    ...
    virtual OutputMinMax GenUniformGrid2D(
        float* out, int xStart, int yStart, int xSize, int ySize, float frequency, int seed) const = 0;

    virtual OutputMinMax GenUniformGrid3D(
        float* out, int xStart, int yStart, int zStart, int xSize, int ySize, int zSize, float frequency, int seed) const = 0;

    virtual OutputMinMax GenUniformGrid4D(float* out, int xStart, int yStart, int zStart, int wStart, int xSize, int ySize,
        int zSize, int wSize, float frequency, int seed) const = 0;

    virtual OutputMinMax GenTileable2D(float* out, int xSize, int ySize, float frequency, int seed) const = 0;

    virtual OutputMinMax GenPositionArray2D(
        float* out, int count, const float* xPosArray, const float* yPosArray, float xOffset, float yOffset, int seed) const = 0;

    virtual OutputMinMax GenPositionArray3D(float* out, int count, const float* xPosArray, const float* yPosArray,
        const float* zPosArray, float xOffset, float yOffset, float zOffset, int seed) const = 0;

    virtual OutputMinMax GenPositionArray4D(float* out, int count, const float* xPosArray, const float* yPosArray,
        const float* zPosArray, const float* wPosArray, float xOffset, float yOffset, float zOffset, float wOffset,
        int seed) const = 0;
...
protected:
    ...
    {
        for (int idx = 0; (size_t) idx < sizeof(PerDimensionVariable<T>::varArray) / sizeof(*PerDimensionVariable<T>::varArray);
             idx++)
        {
            MemberVariable member;
            member.name = nameDesc.name;
    ...
    void AddPerDimensionGeneratorSource(NameDesc nameDesc, U&& func)
    {
        ...
        for (int idx = 0; (size_t) idx < sizeof(PerDimensionVariable<GeneratorSourceT>::varArray) /
                                             sizeof(*PerDimensionVariable<GeneratorSourceT>::varArray);
             idx++)
        {
            MemberNodeLookup member;
            member.name = nameDesc.name;
    ...
    void AddPerDimensionHybridSource(NameDesc nameDesc, float defaultV, U&& func)
    {
        ...
        for (int idx = 0; (size_t) idx < sizeof(PerDimensionVariable<HybridSourceT>::varArray) /
                                             sizeof(*PerDimensionVariable<HybridSourceT>::varArray);
             idx++)
        {
            MemberHybrid member;
            member.name = nameDesc.name;

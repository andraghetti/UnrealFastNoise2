...
{
public:
    void SetJitterModifier(SmartNodeArg<> gen)
    {
        this->SetSourceMemberVariable(mJitterModifier, gen);
    }
    void SetJitterModifier(float value)
    {
        mJitterModifier = value;
    }
...
struct MetadataT<Cellular> : MetadataT<Generator>
{
    MetadataT()
    {
        ...
        this->AddHybridSource({"Jitter Modifier", "Above 1.0 will cause grid artifacts"}, 1.0f, &Cellular::SetJitterModifier,
            &Cellular::SetJitterModifier);
        this->AddVariableEnum(
            {"Distance Function", "How distance to closest cells is calculated\nHybrid is EuclideanSquared + Manhattan"},
...
public:
    ...
    {
        mValueIndex = std::min(std::max(value, 0), kMaxDistanceCount - 1);
    }
...
struct MetadataT<CellularValue> : MetadataT<Cellular>
{
    ...
    {
        this->AddVariable(
            {"Value Index", "Nth closest cell"}, 0, &CellularValue::SetValueIndex, 0, CellularValue::kMaxDistanceCount - 1);
...
public:
    ...
    void SetDistanceIndex0(int value)
    {
        mDistanceIndex0 = std::min(std::max(value, 0), kMaxDistanceCount - 1);
    }
    void SetDistanceIndex1(int value)
    {
        mDistanceIndex1 = std::min(std::max(value, 0), kMaxDistanceCount - 1);
    }
...
struct MetadataT<CellularDistance> : MetadataT<Cellular>
{
    ...
    MetadataT()
    {
        this->AddVariable({"Distance Index 0", "Nth closest cell"}, 0, &CellularDistance::SetDistanceIndex0, 0,
            CellularDistance::kMaxDistanceCount - 1);
        this->AddVariable({"Distance Index 1", "Nth closest cell"}, 1, &CellularDistance::SetDistanceIndex1, 0,
            CellularDistance::kMaxDistanceCount - 1);
        this->AddVariableEnum({"Return Type", "How to combine Index 0 & Index 1"}, CellularDistance::ReturnType::Index0,
            &CellularDistance::SetReturnType, "Index0", "Index0Add1", "Index0Sub1", "Index0Mul1", "Index0Div1");
...
public:
    ...
    void SetLookup(SmartNodeArg<> gen)
    {
        this->SetSourceMemberVariable(mLookup, gen);
    }
    void SetLookupFrequency(float freq)
    {
        mLookupFreq = freq;
    }

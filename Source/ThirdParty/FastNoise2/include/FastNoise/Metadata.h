...
struct FASTNOISE_API Metadata
{
    ...
    static const Metadata& Get()
    {
        ...
        static_assert(std::is_member_function_pointer<decltype(&T::GetMetadata)>::value,
            "Cannot get Metadata for abstract node class, use a derived class, for example: Fractal -> FractalFBm");
...
struct FASTNOISE_API NodeData
{
    ...
    {
        return metadata == rhs.metadata && variables == rhs.variables && nodeLookups == rhs.nodeLookups && hybrids == rhs.hybrids;
    }
};
}    // namespace FastNoise
